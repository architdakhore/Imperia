#!/usr/bin/env bash
# imperia CLI - It was needed, so here it is. lol
...
QS_BIN="${imperia_QS:-qs}"
NIXGL_BIN="${imperia_NIXGL:-}"
...
show_help() {
	cat <<EOF
imperia CLI - Desktop Environment Control

Usage: imperia [COMMAND]

Commands:
    (none)                            Launch imperia
    update                            Update imperia
    ...
    version, -v, --version            Show imperia version
    goodbye                           Uninstall imperia :(

Examples:
    imperia brightness 75              Set all monitors to 75%
    imperia brightness 50 HDMI-A-1     Set HDMI-A-1 to 50%
    imperia brightness +10             Increase brightness by 10%
    imperia brightness -5 HDMI-A-1     Decrease HDMI-A-1 brightness by 5%
    imperia brightness 10 -s           Save current, then set all to 10%
    imperia brightness -s HDMI-A-1     Save current brightness of HDMI-A-1
    imperia brightness -r              Restore saved brightness
...
find_imperia_pid_cached() {
	...
	pid=$(find_imperia_pid)
	echo "$pid"
}

restart_imperia() {
	PID=$(find_imperia_pid_cached)
	if [ -n "$PID" ]; then
		echo "Stopping imperia (PID $PID)..."
		kill "$PID"
...
case "${1:-}" in
update)
	echo "Updating imperia..."
	curl -fsSL get.axeni.de/imperia | sh
	restart_imperia
	;;
refresh)
	echo "Refreshing imperia profile..."
	exec nix profile upgrade imperia --refresh --impure
	;;
run)
	CMD="${2:-}"
	PIPE="/tmp/imperia_ipc.pipe"
	...
	PID=$(find_imperia_pid_cached)
	if [ -z "$PID" ]; then
		echo "Error: imperia is not running"
		exit 1
	fi

	qs ipc --pid "$PID" call imperia run "$CMD" 2>/dev/null || {
...
lock)
	...
	qs ipc --pid "$PID" call imperia run lockscreen 2>/dev/null || {
...
screen)
	...
	if [ "$SUB" = "off" ]; then
		...
		echo "Usage: imperia screen [on|off]"
		exit 1
	fi
	;;
suspend)
...
brightness)
	...
	BRIGHTNESS_SAVE_FILE="/tmp/imperia_brightness_saved.txt"
	...
	if [[ "$ARG2" =~ ^[0-9]+$ ]]; then
		...
		echo "Run 'imperia help' for usage information"
		exit 1
	fi
	...
	;;
version | -v | --version)
	echo "imperia $(cat "${SCRIPT_DIR}/version")"
	;;
goodbye)
	echo "Uninstalling imperia..."
	...
	if [ -f /etc/NIXOS ]; then
		if nix profile list 2>/dev/null | grep -q "imperia"; then
			echo "Removing from nix profile..."
			nix profile remove imperia
		elif command -v imperia >/dev/null 2>&1; then
			echo "imperia was declared in this system. Please remove it from your configuration in order to uninstall."
		else
			echo "imperia is not installed."
		fi
		exit 0
	fi
	...
	rm -rf "$HOME/.local/src/imperia"
	rm -rf "$HOME/.local/share/imperia"
	rm -rf "$HOME/.local/state/imperia"

	if [ "$REMOVE_CONFIG" = true ]; then
		rm -rf "$HOME/.config/imperia"
		echo "Configuration files removed."
	fi

	echo "imperia uninstalled. :("
	;;
help | --help | -h)
	show_help
	;;
