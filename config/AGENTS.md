...
## OVERVIEW
The configuration system in imperia is a reactive, file-backed architecture built on `Quickshell.Io`. 
It serves as the source of truth for all shell modules (Bar, AI, Theme, etc.), managing state through 
synchronized JSON files stored in `~/.config/imperia/config/`. 
