...
Item {
    ...
    // Roman numeral converter (Caelestia-inspired aesthetic enhancement)
    function toRoman(num) {
        if (num <= 0 || num > 20) return String(num);
    ...
    function getWorkspaceLabel(id) {
        if (Config.workspaces?.romanNumerals ?? false) return toRoman(id);
        return String(id);
    }
    ...
    WheelHandler {
        onWheel: event => {
            const maxWs = Config.workspaces.shown;
            const curId = monitor?.activeWorkspace?.id ?? 1;
            if (event.angleDelta.y < 0) {
                if (curId < maxWs) Hyprland.dispatch(`workspace r+1`);
            } else if (event.angleDelta.y > 0) {
                if (curId > 1) Hyprland.dispatch(`workspace r-1`);
            }
        }
            ...
            Button {
                ...
                background: Item {
                    ...
                    property var mainAppIconSource: {
                        const cls = focusedWindow?.class ?? "";
                        if (!cls) return "image://icon/application-x-executable";
                        // 1. Try Quickshell desktop entry resolution
                        const qsIcon = AppSearch.getCachedIcon(cls);
                        if (qsIcon && qsIcon !== "image-missing") {
                            const qsPath = Quickshell.iconPath(qsIcon, "");
                            if (qsPath) return qsPath;
                        }
                        // 2. Fall back to IconUtils smart chain
                        return IconUtils.resolveIconExtended(cls);
                    }
                    ...
                    Text {
                        ...
                        text: getWorkspaceLabel(button.workspaceValue)
                        elide: Text.ElideRight
                        color: (monitor?.activeWorkspace?.id == button.workspaceValue) ? Colors.primary : (workspaceOccupied[index] ? Colors.secondary : Colors.outline)
                    ...
                    Rectangle {
                        ...
                        height: width
                        radius: width / 2
                        color: (monitor?.activeWorkspace?.id == button.workspaceValue) ? Colors.primary : Colors.outline
                ...
                // Caelestia-style hover scale on workspace button
                scale: button.hovered ? 1.15 : (button.pressed ? 0.9 : 1.0)
                Behavior on scale {
                    NumberAnimation {
                        duration: button.pressed ? 80 : Config.animDuration / 3
                        easing.type: button.pressed ? Easing.OutCubic : Easing.OutBack
                        easing.overshoot: 1.4
                    }
                }
            ...
            Button {
                ...
                background: Item {
                    ...
                    Text {
                        ...
                        font.family: Config.theme.font
                        font.pixelSize: workspaceLabelFontSize(text)
                        text: getWorkspaceLabel(buttonVert.workspaceValue)
                        elide: Text.ElideRight
                        color: (monitor?.activeWorkspace?.id == buttonVert.workspaceValue) ? Colors.primary : (workspaceOccupied[index] ? Colors.secondary : Colors.outline)
                    ...
                    Rectangle {
                        ...
                        height: width
                        radius: width / 2
                        color: (monitor?.activeWorkspace?.id == buttonVert.workspaceValue) ? Colors.primary : Colors.outline
