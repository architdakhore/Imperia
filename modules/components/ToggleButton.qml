...
Button {
    ...
    property color iconColor: Colors.primary   // Default: theme primary color
    // Radius handling
    ...
    contentItem: Item {
        ...
        Text {
            ...
            font.pixelSize: 18
            color: root.pressed ? Colors.background : root.iconColor
            horizontalAlignment: Text.AlignHCenter
            verticalAlignment: Text.AlignVCenter

            Behavior on color {
                enabled: (Config.animDuration ?? 0) > 0
                ColorAnimation { duration: (Config.animDuration ?? 0) / 3 }
            }
        }
    ...
    // ── Caelestia-style hover scale & press feedback ──────────────────────────
    scale: root.pressed ? 0.92 : (root.hovered ? 1.08 : 1.0)
    Behavior on scale {
        NumberAnimation {
            duration: root.pressed ? 80 : (Config.animDuration ?? 200) / 3
