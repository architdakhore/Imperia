...
Rectangle {
    ...
    Rectangle {
        ...
        property string selectedCategory: "All"   // ← category filter
        ...
        function updateFilteredApps() {
            var all = searchText.length > 0
                ? AppSearch.fuzzyQuery(searchText)
                : AppSearch.getAllApps();

            // Category filter
            if (appLauncher.selectedCategory !== "All") {
                var cat = appLauncher.selectedCategory;
                filteredApps = all.filter(function(a) {
                    return a.categories && a.categories.indexOf(cat) >= 0;
                });
            } else {
                filteredApps = all;
            }
        ...
        Item {
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    // ── Imperia-style hover scale ──────────────────────────
                    HoverHandler { id: delegateHover }
                    scale: (delegateHover.hovered && !isExpanded) ? 1.015 : 1.0
                    Behavior on scale {
                        enabled: Config.animDuration > 0
                        NumberAnimation {
                    ...
                    RowLayout {
                        ...
                        Item {
                            ...
                            Image {
                                ...
                                property var _sources: IconUtils.iconSources(appIcon)
                                property int _srcIdx: 0

                                // Reset src index whenever _sources changes (fires on delegate
                                // reuse because appIcon → _sources is a live binding)
                                on_SourcesChanged: {
                                    _srcIdx = 0;
                                }

                                source: (_srcIdx >= 0 && _srcIdx < _sources.length)
                                        ? _sources[_srcIdx] : ""
                        ...
                        Column {
                            ...
                            Text {
                                ...
                                elide: Text.ElideRight
                                visible: text !== "" && appLauncher.searchText.length === 0
