...
Item {
    ...
    // Hover expansion disabled: notch pill shows on hover but content stays compact.
    // Only expands when a panel is open or navigating.
    readonly property bool expandedState: (notchHovered && false) || parentHoverActive || isNavigating || Visibilities.playerMenuOpen
    ...
    // Computed dimensions
    readonly property real mainRowContentWidth: 200 + userInfo.width + separator1.width + (mainRow.spacing * 2) + mainRowMargin
    ...
    // Quick stats removed for cleaner notch
    ...
    implicitHeight: {
        let h = mainRowHeight;
        if (hasActiveNotifications) h += notificationContainerHeight;
        return h;
    }
    ...
    Item {
        ...
        Row {
            ...
            CompactPlayer {
                anchors.verticalCenter: parent.verticalCenter
                width: parent.width - userInfo.width - separator1.width - (parent.spacing * 2)
                height: 32
                player: activePlayer
