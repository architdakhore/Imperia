...
PanelWindow {
    ...
    WlrLayershell.namespace: "imperia:wallpaper"
    ...
    property string currentWallpaper: {
        // Use saved wallpaper path immediately on boot (before scan completes)
        if (!initialLoadCompleted && wallpaperConfig.adapter.currentWall)
            return wallpaperConfig.adapter.currentWall;
        if (initialLoadCompleted && wallpaperPaths.length > 0)
            return wallpaperPaths[currentIndex];
        return "";
    }
    ...
    property string colorPresetsDir: Quickshell.env("HOME") + "/.config/imperia/colors"
    ...
    property string mpvSocket: "/tmp/imperia_mpv_socket"
