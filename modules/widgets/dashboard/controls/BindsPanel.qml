...
Item {
    ...
    property string currentCategory: "imperia"
    ...
    property bool isEditingImperia: false
    ...
    function openEditDialog(bind, index, isImperia) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingImperia = isImperia;

        // Initialize edit form state
        if (isImperia) {
            // imperia binds still use old format (single key)
    ...
    function saveEdit() {
        if (root.isEditingImperia) {
            // Save imperia bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["imperia", "section"?, "bindName"]
            
            const adapter = Config.keybindsLoader.adapter;
            if (adapter && adapter.imperia) {
                let bindObj = null;
                if (path.length === 2) {
                    // Top level: imperia.bindName
                    bindObj = adapter.imperia[path[1]];
                } else if (path.length === 3) {
                    // Nested: imperia.system.bindName
                    bindObj = adapter.imperia[path[1]][path[2]];
                }
    ...
    // Get imperia binds as a flat list
    function getimperiaBinds() {
        const adapter = Config.keybindsLoader.adapter;
        if (!adapter || !adapter.imperia)
            return [];

        const binds = [];
        const imperia = adapter.imperia;

        // Core imperia binds (Launcher, Dashboard, etc.)
        ...
        if (imperia.system) {
            ...
            for (const key of systemKeys) {
                if (imperia.system[key]) {
                    binds.push({
                        category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "imperia.system." + key,
                        bind: imperia.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            Repeater {
                id: imperiaRepeater
                model: root.currentCategory === "imperia" ? root.getimperiaBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "imperia" && imperiaRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "imperia" ? "No imperia binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingImperia && !root.isCreatingNew
                        ...
                        // Reset button (only for imperia binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingImperia
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingImperia && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["imperia", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        
                                        // Use the new helper in Config.qml to get the default values
                                        const defaultBind = Config.keybindsLoader.adapter.getimperiaDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for imperia binds only)
                        Text {
                            visible: root.isEditingImperia && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingImperia
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for imperia)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingImperia - Removed to allow editing flags for imperia binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingImperia
                            ...
                            StyledRect {
                                ...
                                radius: Styling.radius(-2)
                                opacity: root.isEditingImperia ? 0.6 : 1.0
                                ...
                                TextInput {
                                    ...
                                    verticalAlignment: Text.AlignVCenter
                                    selectByMouse: true
                                    readOnly: root.isEditingImperia
                                    onTextChanged: {
                                        if (root.editActions.length > root.currentActionPage) {
                                            const currentAction = root.editActions[root.currentActionPage];
